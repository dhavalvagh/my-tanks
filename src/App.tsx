...
function App() {
  ...
  const tankName = (id: string) =>
    state.tanks.find((t) => t.id === id)?.name ?? "Unknown tank";
  ...
  function handleDeleteTank(tankId: string) {
    setState((prev) => {
      ...
      const nextSelected =
        selectedTankId === tankId ? (tanks[0]?.id ?? null) : selectedTankId;
  ...
  const selectedTank = selectedTankId
    ? state.tanks.find((t) => t.id === selectedTankId)
    : null;
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  The water volume calculation starts with the tank&apos;s outer
                  dimensions and makes adjustments.
                </p>
              </CardHeader>
              <CardContent className="space-y-4 text-sm text-muted-foreground">
                ...
                <div>
                  ...
                  <ul className="mt-2 list-disc space-y-2 pl-5">
                    <li>
                      <strong>Headspace:</strong> reduces height (gap from
                      waterline to rim)
                    </li>
                    <li>
                      <strong>Substrate:</strong> reduces height (gravel/sand
                      depth)
                    </li>
                    <li>
                      <strong>Glass thickness:</strong> reduces all dimensions
                ...
                <div className="rounded-md border-l-4 border-info bg-info/10 px-3 py-3">
                  ...
                  <p className="mt-1 text-foreground">
                    30×18×20 cm tank with 2.5 cm headspace, 2 cm substrate, and
                    0.15 cm glass = <strong>8L</strong>
                  </p>
                </div>
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">
                        3–5× tank volume per hour (e.g., 8L = 24–40 L/h)
                      </p>
                    </div>
                    <div>
                      <div className="font-semibold text-foreground">
                      ...
                      <p className="text-sm">
                        5–8× tank volume per hour (e.g., 8L = 40–64 L/h)
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <Suspense fallback={<LoadingFallback />}>
          {(() => {
            if (activeView === "fish-detail") {
              const selectedFish = selectedFishId
                ? state.fishes.find((f) => f.id === selectedFishId)
                : null;
              if (!selectedFish) {
                return (
                  <Card>
              ...
              const fishTank = state.tanks.find(
                (t) => t.id === selectedFish.tankId,
              );
              if (!fishTank) {
                return (
                  <Card>
              ...
              if (!selectedTank) {
                return (
                  <Card>
                    <CardContent>
                      <p className="text-muted-foreground">
                        No tank selected. Add or choose a tank from the menu.
                      </p>
                    </CardContent>
                  </Card>
                );
            ...
            if (activeView === "tanks") {
              return (
                <TankManager
                  ...
                  onOpenDetail={handleOpenTankDetail}
                />
              );
            }
            ...
            if (activeView === "fish") {
              return (
                <FishManager
                  tanks={state.tanks}
                  fishes={state.fishes}
                  editFishId={editingFishId}
                  userId={userId}
                  onSave={handleSaveFish}
                  onDelete={handleDeleteFish}
                  onUploadImage={handleUploadImage}
                  tankNameLookup={tankName}
                />
              );
            }
                    ...
                    setState((prev) => ({
                      ...
                      reminders: [
                        ...prev.reminders,
                        {
                          ...r,
                          id: crypto.randomUUID(),
                          nextDue: Date.now() + r.everyHours * 60 * 60 * 1000,
                          enabled: true,
                        },
                      ],
                    }))
